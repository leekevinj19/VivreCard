        ...
        NavigationStack {
            ScrollView {
                VStack(spacing: 20) {
                        ...
                        Button {
                            ...
                            ZStack(alignment: .topTrailing) {
                                Image(systemName: "envelope.fill")
                                    .foregroundColor(firebase.incomingRequests.isEmpty ? .textSecondary.opacity(0.4) : .sunsetOrange)
                                    .font(.system(size: 18, weight: .semibold))
                                if !firebase.incomingRequests.isEmpty {
                                    Text("\(firebase.incomingRequests.count)")
                                        .font(VivreFont.label(9))
                                        .foregroundColor(.white)
                                        .padding(3)
                                        .background(Circle().fill(Color.strawHatRed))
                                        .offset(x: 8, y: -6)
                                }
                            }
                        }
                    ...
                    // Incoming requests banner (extra nudge when requests exist)
                    if !firebase.incomingRequests.isEmpty {
            ...
            .onAppear {
                // Re-attach listeners every time this tab appears
                firebase.listenToRequests()
                firebase.listenToFriends()
            }
